package rpc11_spi.provider;

import com.caucho.hessian.io.Hessian2StreamingInput;
import com.caucho.hessian.io.Hessian2StreamingOutput;
import rpc11_spi.provider.spi.FindImplByJdkSpi;
...
//                    });
    ...
    private static void processOPRead(SelectionKey selectionKey) throws Exception {
        ...
        while(socketChannel.read(byteBuffer) > 0){
            ...
            // 从消费者获取的接口名，换成服务提供者的接口名，这里是临时实现------------------------
            interfaceName = interfaceName.replaceAll("consumer\\.","provider.");
            // 通过接口名获取实现类原信息 强制转换成响应的实现类
            Class<?> clazz;
            // 用spi机制实现服务实现类的加载，并放入cache中。
            clazz = FindImplByJdkSpi.getInterfaceImpl(interfaceName);
